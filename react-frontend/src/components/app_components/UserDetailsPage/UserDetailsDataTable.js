...
const UserDetailsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_numberTemplate1 = (rowData, { rowIndex }) => <p >{rowData.phoneNo}</p>
...
<Column field="phoneNo" header="phoneNo" body={p_numberTemplate1} filter={selectedFilterFields.includes("phoneNo")} hidden={selectedHideFields?.includes("phoneNo")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload UserDetails Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search UserDetails" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
      <Dialog
        header="Hide Columns"
